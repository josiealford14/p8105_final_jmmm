...
drug_price = 
  drug_price %>%
  rename(drug_name = ndc_description,
         drug_type = classification_for_rate_setting,
         unit_price = nadac_per_unit) %>%
  separate(effective_date, into = c("year", "month", "date")) %>%
  mutate(grams = str_replace_all(drug_name,"[A-Z]", ""),
         drug_name = str_replace(drug_name, "[0-9]", "_")) %>% 
  separate(drug_name, into = c("drug_name", "remove"), sep = "_") %>% 
  mutate(drug_name = tolower(drug_name),
         otc = as.factor(otc)) %>% 
  select(-c(remove, date))
drug_price
```
...
orange_book = left_join(product_ingredient, patent_exclusivity, by.product_ingredient = "appl_no")
...
orange_book_clean = orange_book_clean %>%
  mutate(drug_application_type = factor(drug_application_type, levels = c("NDA_or_innovator", "ANDA_or_generic"), labels = c("B", "G")))
```
Josie
```{r}
#further cleaning of drug_price
drug_price_new = drug_price %>%
  mutate(drug_name = as.character(drug_name)) %>%
  mutate(active_ingredient = str_to_title(drug_name)) %>%
  select(-drug_name)
#inner join of disease drug and orange book =y by trade_name
orange_book_clean_new = inner_join(drug_disease_data_new, orange_book_clean, by = "trade_name")
#left join of y and nih funding by disease_name
final_dataset = left_join(orange_book_clean_new, final_NIH_funding, by = "disease")
final_dataset
```
...
Muhire
```{r}
#scrape drugs info with iteration
