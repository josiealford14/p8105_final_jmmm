...
#Which pharmaco's are holding the most patents?
most_pat_company = final_dataset %>% 
  select(-c(1:13, 15:16, 18:24, 26:31)) %>% 
...
most_pat_company %>% 
  ...
  plot_ly(x = ~appfull, y = ~num_pat_held, color = ~appfull, type = "bar", colors = "Set3")

#diseases with most patents issued
most_pat_dis = final_dataset %>% 
  select(-c(1:16, 18:24, 26:31)) %>% 
  filter(!duplicated(patent_num)) %>% 
  group_by(disease) %>% 
  count() %>% 
  rename(num_pat_held = n) %>% 
...
# #scatter year_pat_expires
# final_dataset %>% 
#   ggplot(aes(x = disease, y = year_pat_expires)) + 
#   geom_point() 
# 
# final_dataset %>% 
#   ggplot(aes(x = disease, y = year_excl_expires)) + 
#   geom_point() 
```
