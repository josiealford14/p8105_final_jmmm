...
* The code chunks below were used for loading and tidying the data.
 ...
 + We joined patents, products, exclusivity, ingredient, drug_disease_data, final_nih_funding, and drug_price and chronic_diseases_final into our Final Dataset named final_dataset. We used the variables ingredient, appl_no, product_no, drug_type, trade_name, otc, and disease as keys for joining (inner and left).
...
#### Dataset 9: Advertising Spending
```{r}
...
Josie
### Do drugs that treat chronic illnesses (diabetes, heart disease, high blood pressure, etc) receive more or less funding, and pricing  compared to acute illnesses? ## How does pricing and funding compare across diseases (27 unique diseases in our "orange book") regardless of disease type?
...
summary_funding_individual_diseases = final_dataset %>%
  ...
  summarize(n = n(), mean = mean(funding), mean = mean(funding), standard_deviation = sd(funding), minimum = min(funding), maximum = max(funding)) 

#Which disease (top 3) has the highest mean funding (by disease type)? 
max_funding_individual_diseases = summary_funding_individual_diseases %>%
  filter(min_rank(desc(mean)) < 4)

#Which disease (bottom 3) has the lowest mean funding (by disease type)? 
min_funding_individual_diseases = summary_funding_individual_diseases %>%
  filter(min_rank(mean) < 4)
...
#Which disease (top 3) has the highest mean funding? 
...
# 1. Diseases with higest mean funding (grouped by disease_type) (top 3) (bar chart)
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding)) %>%
   mutate(mean_funding = mean(funding),
          disease = fct_reorder(disease, mean_funding)) %>%
  filter(min_rank(desc(mean)) < 4) %>%
  plot_ly(x = ~disease, y = ~mean, color = ~disease_type, type = "bar")
  
  
# 2. Diseases with lowest mean funding (bottom 3) (bar chart)
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding)) %>%
  group_by(disease_type, disease) %>% 
  summarize(mean = mean(funding)) %>%
  filter(min_rank(mean) < 4) %>%
  plot_ly(x = ~disease, y = ~mean, color = ~disease_type, type = "bar", colors = "Set2")

# 1A. Overall: Diseases with highest mean funding
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding)) %>%
   mutate(disease = fct_reorder(disease)) %>%
  group_by(disease) %>% 
  summarize(mean = mean(funding)) %>%
  filter(min_rank(desc(mean)) < 6) %>%
  plot_ly(x = ~disease, y = ~mean, type = "bar", color = ~disease, colors = "Set2")

# 2A. Overall: Diseases with lowest mean funding
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding)) %>%
  group_by(disease) %>% 
  summarize(mean = mean(funding)) %>%
  filter(min_rank(mean) < 6) %>%
  plot_ly(x = ~disease, y = ~mean, type = "bar", color = ~disease, colors = "Set2")

# 1B. Acute Diseases with highest mean funding
final_dataset %>%
...
# 1C. Chronic Diseases with highest mean funding
final_dataset %>%
...
# 2B. Acute Diseases with lowest mean funding
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding) & disease_type == "acute") %>%
  group_by(disease) %>% 
  summarize(mean = mean(funding)) %>%
  filter(min_rank(mean) < 6) %>%
  plot_ly(x = ~disease, y = ~mean, type = "bar", color = ~disease, colors = "Set1")

# 2C. Chronic Diseases with lowest mean funding
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding) & disease_type == "chronic") %>%
  group_by(disease) %>% 
  summarize(mean = mean(funding)) %>%
  filter(min_rank(mean) < 6) %>%
  plot_ly(x = ~disease, y = ~mean, type = "bar", color = ~disease, colors = "Set3")
...
# 5. Prevalence vs. mean Funding based on Disease Type. Do more prevalent Chronic or Acute Diseases receive more funding?
#line graph
final_dataset %>%
  filter(!is.na(disease) & !is.na(funding)) %>%
  group_by(disease_type, prevalence) %>% 
  summarize(mean_funding = mean(funding)) %>%
  plot_ly(x = ~prevalence, y = ~mean_funding, type = "scatter", mode = "lines", color = ~disease_type, colors = "Set1")
...
final_dataset %>%
  ...
  summarize(mean_funding = mean(funding)) %>%
  plot_ly(x = ~prevalence, y = ~mean_funding, type = "scatter", color = ~disease_type, colors = "Set1")
```
...
summary_pricing_individual_diseases = final_dataset %>%
  ...
  summarize(n = n(), mean = mean(avg_price), mean = mean(avg_price), standard_deviation = sd(avg_price), minimum = min(avg_price), maximum = max(avg_price)) 

#Which disease (top 3) has the highest mean pricing (by disease type)? 
max_price_individual_diseases = summary_pricing_individual_diseases %>%
  filter(min_rank(desc(mean)) < 4)

#Which disease (bottom 3) has the lowest mean pricing (by disease type)? 
...
#Which disease (top 3) has the highest mean pricing? 
...
# 1. Diseases with higest mean pricing (grouped by disease_type) (top 3) (bar chart) NEED HELP REARRANGING FROM SMALLEST TO LARGEST
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  group_by(disease_type, disease) %>% 
  summarize(mean = mean(avg_price)) %>%
  filter(min_rank(desc(mean)) < 4) %>%
  plot_ly(x = ~disease, y = ~mean, color = ~disease_type, type = "bar")
  
# 2. Diseases with lowest mean pricing (bottom 3) (bar chart)
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  group_by(disease_type, disease) %>% 
  summarize(mean = mean(avg_price)) %>%
...
# 1A. Overall: Diseases with highest mean Pricing
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
...
# 2A. Overall: Diseases with lowest mean Pricing
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  group_by(disease) %>% 
  summarize(mean = mean(avg_price)) %>%
...
# 1B. Acute Diseases with highest mean Pricing
final_dataset %>%
...
# 1C. Chronic Diseases with highest mean Pricing
final_dataset %>%
...
# 2B. Acute Diseases with lowest mean Pricing
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price) & disease_type == "acute") %>%
  group_by(disease) %>% 
  summarize(mean = mean(avg_price)) %>%
...
# 2C. Chronic Diseases with lowest mean Pricing
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price) & disease_type == "chronic") %>%
  group_by(disease) %>% 
  summarize(mean = mean(avg_price)) %>%
...
# 5. Prevalence vs. mean Pricing based on Disease Type. Do more prevalent Chronic or Acute Diseases have higher average prices?
#line graph
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  group_by(disease_type, prevalence) %>% 
  summarize(mean_avg_price = mean(avg_price)) %>%
  plot_ly(x = ~prevalence, y = ~mean_avg_price, type = "scatter", mode = "lines", color = ~disease_type, colors = "Set1")
  
#scatter plot. the less prevalent a disease, the higher the mean_avg price
final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  group_by(disease_type, disease, prevalence) %>% 
  summarize(mean_avg_price = mean(avg_price)) %>%
  plot_ly(x = ~prevalence, y = ~mean_avg_price, type = "scatter", color = ~disease_type, colors = "Set1")
```
...
summary_advertising_pricing = final_dataset %>%
  ...
  summarize(mean_avg_price = mean(avg_price)) 
# Xeljanz, which treats both arthritis and rheumatoid arthritis (chronic conditions), has the highest mean average price of $43.14. Eliquis, which treats atrial fibrillation, stroke, and thromboembolism (both chronic and acute conditons), had the lowest mean average price of $5.28. 
...
final_dataset %>%
  ...
  group_by(trade_name, disease, disease_type) %>%
  plot_ly(y = ~avg_price, type = "box", color = ~disease_type, colors = "Set3") %>%
  layout(title = "Distribution of Average Price for Top Advertised Drugs based on Disease Type")
# Top advertised drugs that treat acute conditons have a lower mean average price compared to top advertised drugs that treat chronic conditons. Overall, top advertised drugs that treat acute conditon may have a statisitcally different average price compared to those that treat chronic illnesses. 
...
final_dataset %>%
  ...
  summarize(mean_avg_price = mean(avg_price)) %>%
  plot_ly(x = ~mean_avg_price, y = ~spending_million_dollars, color = ~trade_name, type = "scatter", colors = "Set2") %>%
  layout(title = "mean Average Price compared to Advertising Spending", xaxis = list(title = 'mean Average Price'), yaxis = list(title = 'Advertising Spending (Million)'))
# No clear trend. Interesting that the pharmaceutical company Bristol Myers Squibb invests a high amount of advertising spending for a low cost drug like Elliquis.  
# It would be interesting to see if Bristol Myers Squibb justifies this because of demand? (cue investigation of drug sales!!!!)
```

###What about drug sales? Let's look at the top 20 prescription drugs based on 2016 US Sales (in billions of dollars)
```{r}
summary_drug_sales = final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  inner_join(top_prescribed_drugs_sales) %>%
  group_by(trade_name, disease, disease_type) %>%
  summarize(mean_drug_sales_billions = mean(sales_billion_dollars))
```
...
final_dataset %>% 
  filter(disease %in% c("Heart Failure")) %>%
