...
### FUNDING
```{r Chronic Disease Analysis: Funding}
...
# 1. Diseases with higest median funding (top 10) (bar chart)

# 2. Diseases withlowest median funding (bottom 10) (bar chart)

# 3. Funding by disease status (boxplot)

# 4. Funding by disease status & disease (boxplot)

# 5. Prevalence vs. Median Funding based on Disease Type. Do more prevalent Chronic or Acute Disease receive more funding?

# 6. US Mortality vs. Median Funding based on Disease Type. Do more deadly Chronic or Acute Disease receive more funding?

#Graph of Funding 
funding_chronic_disease = final_dataset %>%
  filter(!is.na(disease)) %>% 
  group_by(chronic_disease) %>% 
```

```{r Chronic Disease Analysis: Pricing of Drugs}
...
```{r}
#there is no method to this madness! somewhat repitive of the chronic disease analysis. i think i'll skip
```
