...
knitr::opts_chunk$set( collapse = TRUE) # courtesy of Julia
```
...
First, we looked to see if there was an overall trend in the average price of all drugs from 2013 to 2017.  We observe a clear increasing trend in the overall average drug price.  In particular, we observe a notable increase between 2014 and 2015. We think this may be related to the full implimentation of the Affordable Care Act (ACA) in 2015.

We also looked at the total NIH funding for each year from 2013-2018.  We see that funding increased until it leveled in 2016 and there is a projected decrease for 2018.
...
final_dataset %>%
  ...
  layout(title = "Trend in Overall Average Drug Price", yaxis = list(title = "Drug Price"))


final_NIH_funding %>%
  group_by(year) %>%
  summarize(avg_funding = sum(funding, na.rm = TRUE)) %>%
  plot_ly(x = ~ year, y = ~ avg_funding, mode = "lines", type = "scatter")

```


Then, we looked to determine whether funding correlated with drug prices.  This does not really seem to be the case from the scatterplot.  Along the entire range of funding amounts, most drugs still have relatively low prices.  It is interesting that different drugs that treat the same disease seem to be largely clustered together.  We can see one breast cancer drug, Afinitor, which actually can be used to treat various types of caners including kidney, pancreas, breast, and brain cancer, has an unusually high price.  We think these other applications are what are influencing the price.  Another unusual drug is Belviq, used to treat obesity.  This drug has a very low price, which we think is because of the extremely high prevalence of obesity or other factors such as competitors. 

Looking at funding vs. disease for acute vs chronic diseases, we see a relatively random scatter. Therefore, we do not think there is a significant trend for funding.
...
final_dataset %>%
  ...
  layout(title = "Average Funding For Chronic vs Acute Diseases", xaxis = ax)
...
```{r drug_price_distr}
trade_name_choice = "Abacavir"
violin_drug = drug_price %>%
  filter(trade_name == trade_name_choice) %>%
  ggplot(aes(x = year, y = unit_price, fill = year)) + 
  geom_violin() +
  ggtitle(paste0("Distribution of Prices for ", trade_name_choice), subtitle = NULL)
...
final_dataset %>% 
  ...
  layout(title = "Average Price of Drugs over Time", showlegend = FALSE)
```
...
final_dataset %>% 
  ...
  layout(title = "Funding vs Need", showlegend = FALSE)
```
...
patents = final_dataset %>% 
  select(-c(1:13, 15:16, 18, 19, 21:24, 26:31)) %>% 
  filter(!duplicated(patent_num)) %>% 
  filter(!is.na(disease)) %>%
  filter(year_pat_expires > 2017) %>% 
  mutate(
    as.factor(year_pat_expires > 2017), 
...
# relationship between excl vs pat


#Which pharmaco's are currently holding the most patents?
...
most_pat_company 
############# QUESTION:  How can we arrange the legend to be alphabetized?
...
#pharmaco that has most patents expiring by year (i.e. in each upcoming year, who has most expiring?  Could this be a driver for funding applications?)
pat_exp = patents %>%
...
dis_pat_exp = patents %>%
  ...
  plot_ly(
    ...
    colors = "Set2")
dis_pat_exp
######################### QUESTION:  How do I amend x-axis to show each year as opposed to every two years?
```
...
##### Funding
* Is there a difference in funding between acute conditons and chronic diseases?

  + Now, let's investigate if there is a difference in distribution of funding based on disease conditon: acute versus chronic. We see that acute conditions have a wider interquartile range compared to chronic conditions: there is greater variability in acute conditon funding compared to chronic. 25% of acute conditons have a funding of $18 million or less. 25% of acute condtions receive funding of more than $313 million or more. The median funding for acute conditons is $50 million per year. Acute funding is right skewed. 
  
  + Comparatively, chronic conditions have a lower variability in funding, but have more outiliers. 25% of chronic conditons have a funding of $224 million or less. 25% of chronic condtions receive funding of more than $272.5 million or more. The median funding for acute conditons is $253 million per year. Chronic funding has an approximately normal distribution in funding. 
...
t.test(acute$funding, chronic$funding, var.equal = TRUE, paired = FALSE)
```

##### Pricing
* Is there a difference in funding between acute conditons and chronic diseases? 

  + Acute conditons have greater variability in pricing compared to chronic diseases, while chronic diseases have exponentionally more outliers in pricing. Acute has a larger IQR, while chronic has a wider range in pricing. Average price of drugs that treat chronic diseases range from $0.03 to $1,096.67 per dose. Average price of drugs that threat chronic diseases range from $0.11 to $462.03. 
  + Is there a statistically significant difference in pricing of drugs that treat either acute or chronic diseases? 
    + Yes; drugs that treat acute conditons have a mean "average price" of $173.43 per dose and chronic diseases have a mean "average price" of $125.39 per dose. At the 0.05 significance level, we reject the null hypothesis that the true difference of mean "average price" between drugs that treat acute or chronic diseases is zero. We are 95% confident that drugs that treat acute conditions are priced between $43.87 and $52.22 more per dose than drugs that treat chronic conditions. 
```{r, warning = FALSE, message = FALSE}
...
chronic = final_dataset %>%
  filter(!is.na(disease) & !is.na(avg_price)) %>%
  filter(disease_type == "chronic")
t.test(acute$avg_price, chronic$avg_price, var.equal = TRUE, paired = FALSE)
```

### **Case Study** 
* We are interested in conducting a case study on pharmaceutical drugs that receive the highest advertisment spending, are the most prescribed according to US sales, are most abused (over prescribed, highly addictive, and/or frequently used as illicit street drugs), and are commonly referenced in everyday life or in popular American culture.
...
```
  ...
  + First we examined the most advertised drugs in 2016.
    ...
    + Eliquis, which treats atrial fibrillation, stroke, and thromboembolism (both chronic and acute conditons), had the lowest mean average price per dose of $5.29.
...
#motivation: statistics on prescription abuse among high schoolers (drugabuse.gov site)
```
  + Now, let's take a look at the most abused drugs in America
    ...
    + This list of drugs involves drugs that are over prescribed, highly addictive, and/or frequently used as illicit street drugs
      ...
      + Stimulants
        ...
        + Ritalin: commonnly abused by college students
      
        + Concerta
      
        + Adderall: commonnly abused by college students
      
  + Interested in how prices for opiod drugs have changed over time, especially in regards to the Opioid Epidemic. 
  
    + Public Health threat. 
    
    + 2 million Americans abused or dependent, 1000+ ER visits daily. 78 deaths daily. (2014)
    + What casued the Epidemic? Perfect Storm
    
        + Practitioners were urged to treat pain aggresively (without the tools and training), pharmaceutical companies marketing prescription opioids to doctors.
      
      + 259 million opioid prescription in 2012, according to the CDC. An increase in 400% since 1999.
      
      + Prescription opioids account for 1/2 of opioid deaths. Considered gateway drugs for heroin
      
      + Highly addictive. Other opioids include morphine, codeine cough syrup (know as "lean", "purple drank", "Sizzurp", and "syrup". Infamously, the rapper Lil' Wayne has struggled with past addiction to codeine. The rapper Pimp C died from overdosing on codeine).
...
```
  + Finally, we investigate "popular" drugs: pharmaceutical drugs that people we know use, ones that we see commericals for, and/or commonly discussed in popular culture
...
#Elliquis spends more than Xeljanz, 
#some drugs are missing because of subsetting
...
final_dataset %>%
  ...
  layout(title = "Distribution of Average Price for Most Abused Drugs")
...
final_dataset %>%
  ...
  group_by(trade_name) %>% 
  plot_ly(y = ~avg_price, type = "box", color = ~trade_name, colors = "Set3") %>%
  layout(title = "Distribution of Average Price for Popular Drugs")

final_dataset %>% #harvoni is an extreme outlier, should we drop?
  filter(!is.na(avg_price)) %>%
  filter(trade_name %in% top_prescribed_drugs_sales) %>%
  group_by(trade_name) %>% 
  plot_ly(y = ~avg_price, type = "box", color = ~trade_name, colors = "Set3") %>%
  layout(title = "Distribution of Average Price for Most Prescribed Drugs")
...
#### **"Pill Pop": The Relationship Between Pop Music & "Pill Popping" Culture**
```{r}
# #focus on xanax and percocet
# # Use Muhire's 
# trade_name_choice = "Xanax"
# violin_drug = drug_price %>%
#   filter(trade_name == trade_name_choice) %>%
#   ggplot(aes(x = year, y = unit_price, fill = year)) + 
#   geom_violin() +
...
#### **The Opioid Epidemic**
```{r}
# focus on pharmaceutical opioids: Duragesic, Vicodin, Opana, Darvon, Dilaudid, Demerol & Lomotil. Have their prices increased over time? 
# mention their "cousins": heroin and synthetic fentanyl
# https://en.wikipedia.org/wiki/Opioid_epidemic
# https://www.forbes.com/sites/neilhowe/2017/11/30/americas-opioid-crisis-a-nation-hooked/#241da82a6a57
#motivation: One in three unintential overdose deaths result from opioids combined with benzodiazepines (Valium and Xanax)
```
...
We observe a clear increasing trend in overall average drug price from 2013 to 2017 wtih a notable increase between 2014 and 2015, possibly due to ACA implimentation in 2015.
