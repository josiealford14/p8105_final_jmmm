...
* We look at the yearly distribution of drug prices for a few drugs. Some drugs have skewed price distributions. Zoloft, a drug that treats panic disorders, seems to have almost doubled from 2013 to 2017.
...
most_pat_company = patents %>%
  ...
  layout(title = "No. of Patents Currently Held by Pharmaceutical Company", xaxis = xax, yaxis = yax)
most_pat_company
```
...
most_pat_dis = patents %>%
  ...
  layout(title = "No. of Patents Currently Held by Disease", xaxis = xax, yaxis = yax)
most_pat_dis
```
...
# pat_exp %>%
#   plot_ly(x = ~year_pat_expires, y = ~num_pat_exp, color = ~appfull, type = "scatter", mode = "lines", colors = "Set2") %>%
#   layout(title = "Projection of Expiring Patents by Pharmaceutical Company ", xaxis = xax_2, yaxis = yax_2)
# 
...
dis_pat_exp = patents %>%
  ...
  # mutate(disease = fct_reorder(disease, num_pat_exp)) %>%
    plot_ly(x = ~year_pat_expires, 
          ...
          marker = list(size = ~num_pat_exp*3), 
          text = ~paste('Count: ', num_pat_exp)) %>%
  layout(title = "Projection of Expiring Patents by Disease", xaxis = xax_3, yaxis = yax_3, showlegend = FALSE)
  dis_pat_exp
```
