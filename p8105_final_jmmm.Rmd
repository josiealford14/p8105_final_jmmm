...
rm(chronic_diseases_1, chronic_diseases_2, chronic_diseases_3) #clean up workspace
...
chronic_diseases_final$disease[71] = "COPD"

# Recode the 15 or so unique "disease" entries from the final_dataset 

# Join final dataset and chronic disease dataset to complete analysis.

# #join final_dataset to chronic_diseases to highlight chronic illnesses
# final_dataset_chronic_match = stringdist_left_join(final_dataset, chronic_diseases_final, by = "disease", distance_col = NULL) %>%
#    as_tibble() %>%
#   filter(!is.na(value))
#    rename()
# final_dataset = fuzzy_left_join(final_dataset, chronic_diseases_final, by = "disease")
# final_dataset$chronic_disease[is.na(final_dataset$chronic_disease)] = "no"
# #join final_dataset to chronic_diseases to highlight chronic illnesses
# # final_dataset_chronic_match = pmatch(final_dataset$disease, chronic_diseases_final$disease) %>%
# #   as_tibble()
# final_dataset_chronic_match = stringdist_left_join(final_dataset$disease, chronic_diseases_final$disease)
# final_dataset = stringdist_left_join(final_dataset, chronic_diseases_final, by = "disease")
# final_dataset$chronic_disease[is.na(final_dataset$chronic_disease)] = "no"
```

### Is  trade name and condition related? (Pseudo Sentiment analysis)
```{r}

```

### For 2016,  for a given drug is how much money is spent on advertising correlated with unit price? (funding for 2016, advertising cost dataset).
```{r}
