...
summary_advertising_pricing = final_dataset %>%
  ...
  summarize(median_avg_price = median(avg_price)) 
# Xeljanz, which treats both arthritis and rheumatoid arthritis (chronic conditions), has the highest median average price of $43.14. Eliquis, which treats atrial fibrillation, stroke, and thromboembolism (both chronic and acute conditons), had the lowest median average price of $5.28. 
...
# Top advertised drugs that treat acute conditons have a lower median average price compared to top advertised drugs that treat chronic conditons. Overall, top advertised drugs that treat acute conditon may have a statisitcally different average price compared to those that treat chronic illnesses. 
...
final_dataset %>%
  ...
  layout(title = "Median Average Price compared to Advertising Spending", xaxis = list(title = 'Median Average Price'), yaxis = list(title = 'Advertising Spending (Million)'))
...
summary_drug_sales = final_dataset %>%
  ...
  group_by(trade_name, disease, disease_type) %>%
  summarize(median_drug_sales_billions = median(sales_billion_dollars))
```

```{r remove_var, eval = FALSE}
